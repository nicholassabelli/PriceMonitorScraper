...
class IGAStripAmountPipeline(StripAmountPipeline):
    def process_item(self, item, spider):
        if item.get(Product.KEY_CURRENT_PRICE) and item.get(Product.KEY_CURRENT_PRICE).get(Offer.KEY_AMOUNT):
            ...
            item[Product.KEY_CURRENT_PRICE][Offer.KEY_AVAILABILITY] = Availability.IN_STOCK.value
        else:
            item[Product.KEY_CURRENT_PRICE][Offer.KEY_AVAILABILITY] = Availability.OUT_OF_STOCK.value
